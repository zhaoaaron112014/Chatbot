...
public class Chatbot
{
    ...
    public String getResponse(String statement){
        messageCounter ++;
        statement = Helper.reduceContractions(statement);

        String response="";

        for (int i = 0; i < Intro.greetings.length; i++){
            if (findKeyword(statement, Intro.greetings[i])>= 0){
                response = Intro.greetingResponse();
                return response;
            }
        }
        ...
        if(statement.length() == 0){
            ...
            System.out.println(Intro.iAmResponse(statement));
        }
